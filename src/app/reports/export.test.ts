...
import { buildSummarySheetRows, buildClientsSheetRows } from "./export";
...
describe("buildSummarySheetRows", () => {
  it("formatuje grosze jako liczby złotych (w tym korekty ujemne)", () => {
    ...
    const rows = buildSummarySheetRows(summary, 2);
    ...
    expect(rows).toContainEqual(["16. Liczba banków / SKOK-ów wśród płacących", 2]);
  });
});
