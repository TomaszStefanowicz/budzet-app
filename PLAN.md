...
- **1.6** Zapis do bazy: import całościowy zastępujący stan, metryki importu, aktualizacja słownika klientów (nowi klienci z typem „nieokreślony"). Rozbite na mniejsze kroki:
  ...
  - [ ] **1.6d** Zapis `revenue_items` + `revenue_months`: poprzednie pozycje `is_active = false` (historia zachowana, decyzja potwierdzona z użytkownikiem), nowe wstawione, całość w transakcji. `revenue_months` to zawsze dosłowny odczyt kolumn miesięcznych z pliku, bez żadnych obliczeń.
